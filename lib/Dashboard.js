...
const Dashboard = () => {
    ...
    useEffect(() => {
        socket.on('message', message => {
            ...
            let train = {};
            train.id = response.id || id;
            train.name = response.name;
            train.from = response.from || "";
            train.to = response.to || "";
            train.current = response.current || "";
            train.status = response.status || "Stopped";
            train.delay = response.status || "On-Time";
            setTrains(trains => [...trains, train])
            let msg = `${response.name} is now available at ${response.city} metro.`
    ...
    const onFormSubmit = (event) => {
        ...
        let message = {
            name, from, current, next, to, status, delay
        }
